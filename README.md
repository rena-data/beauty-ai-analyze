...
- 모델 폴백 체인: 503/429 시 자동으로 다른 모델로 전환
- API 키 이중화: 할당량 초과 시 백업 키로 자동 전환
- 얼굴 없는 사진은 프롬프트 내에서 감지 후 에러 반환
...
| 2026.05.06 | 모델 폴백 체인 (4개 모델 순차 시도) / API 키 이중화 / 웹 UI 전면 고도화 (4계절 비교 카드, 레이더 차트, 메이크업 포인트 카드, 컬러 팔레트 3행) / 패션 매칭 기능 (옷 사진 → S/A/B/C 등급) / 퍼스널컬러 퀴즈 게임 (15명 연예인) / Supabase 연동 (분석 통계, 클릭 추적, 결과 공유 URL) / SEO 메타태그 + Google Search Console 등록 |
