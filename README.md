...
- **스타일링 제안**: 메이크업 포인트 카드(베이스/아이섀도/치크/립) + 헤어컬러 추천 + 패션 색 조합 + 악세사리 가이드
...
- **구매링크**: 올리브영/무신사 제품명 검색 URL 연결 (브랜드 제외, # 이전까지)
...
| 2026.06.04 | 상세분석 UI 개선(시즌 히어로 배너+아이콘 metric-card), buy_url 검색 URL 전환(286개), 리포트 아이섀도/립 컬러 하이라이트 개선, 컬러 팔레트 데이터 전면 수정(섀도 베이스/worst 다양화), 보안 강화(CORS/Rate Limiting) |
| 2026.06.04 | 2차 개선 — 컬러칩 정렬 수정, 레이더 차트 텍스트 잘림 해결, worst 컬러 5개 확대, 컬러 팔레트 상세 설명 추가(섀도 베이스/메인/포인트/뉴트럴), 스타일링 탭 중복 팔레트 제거, 리포트 팔레트 안내문·역할 라벨 추가, GitHub Contributors 정리 |
